...
# This dinner is good!
def not_bad(s):
    posCadenaNot = s.find("not")
    posCadenaBad = s.find("bad")
    if posCadenaNot < posCadenaBad:
        # Not esta primero
        s = s[:posCadenaNot] + "good" + s[(posCadenaBad + 3):]
    return s
...
def front_back(a, b):
    ...
    s = ""
    if (evaluaCadenaPar(a) and evaluaCadenaPar(b)):
        s = a[:(len(a) / 2)] + b[:(len(b) / 2)] + a[(len(a) / 2):] + b[(len(b) / 2):]
    elif(not(evaluaCadenaPar(a)) and evaluaCadenaPar(b)):
        s = a[:(len(a) / 2) + 1] + b[:(len(b) / 2)] + a[(len(a) / 2) + 1:] + b[(len(b) / 2):]
    elif(evaluaCadenaPar(a) and not(evaluaCadenaPar(b))):
