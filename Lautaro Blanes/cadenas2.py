...
def not_bad(s):
    cadena = s
    subcadenaNot = cadena.find("not")
    subcadenaBad = cadena.find("bad")
    if subcadenaNot < subcadenaBad:
        cadena = cadena[0:subcadenaNot] + "good" + cadena[subcadenaBad+3:]
    return cadena
...
def front_back(a, b):
    ...
    divisionB = len(b) / 2
    if len(a) % 2 != 0:
            divisionA = divisionA + 1
            primeraA = a[:divisionA]
            finalA = a[divisionA:]
    else:
        primeraA = a[:divisionA]
        finalA = a[divisionA:]
    if len(b) % 2 != 0: 
            divisionB = divisionB + 1	
            primeraB = b[:divisionB]
            finalB = b[divisionB:]
    else:	
        primeraB = b[:divisionB]
        finalB = b[divisionB:]
    cadena = primeraA + primeraB + finalA + finalB
    return cadena
